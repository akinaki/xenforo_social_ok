...
class Social_Manufacture
{
	...
	protected function _installVersion1()
	{
		...
		// OK
		if(!$db->fetchRow('SHOW columns FROM xf_user_profile WHERE field = \'ok_auth_id\''))
		{
			$db->query("
				ALTER TABLE xf_user_profile
				ADD ok_auth_id BIGINT(20) UNSIGNED NOT NULL DEFAULT 0 AFTER twitter_auth_id
			");
		}
	}
	...
	protected function _uninstallStep1()
	{
		...
		$db->query("
                ...
                WHERE provider = 'vk' OR provider = 'twitter' OR provider = 'google' OR provider = ''
      		");
	}
}